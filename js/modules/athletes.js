    ...
    ((listContentEl.innerHTML = `
      ...
      <div class="card" style="cursor:pointer;position:relative;overflow:hidden;" data-athlete-id="${Utils.escapeHtml(ath.id)}" data-name="${Utils.escapeHtml((ath.full_name || "").toLowerCase())}" data-role="${Utils.escapeHtml((ath.role || "").toLowerCase())}" data-team="${Utils.escapeHtml((ath.team_name || "").toLowerCase())}">
        ...
        <div style="display:flex;align-items:flex-start;gap:var(--sp-2);">
          <div style="width:48px;height:48px;background:${initialsColor};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-family:var(--font-display);font-weight:700;font-size:1.3rem;color:#000;border-radius:8px;">${
            ath.photo_path
          ...
          !(async function (targetEl) {
            ...
            try {
              ...
              const athleteRows = (metricsAthletes || [])
                .map(
                  (athlete) =>
                    `<tr style="cursor:pointer;" data-athlete-id="${athlete.id}"><td><div style="display:flex;align-items:center;gap:10px;"><div style="width:32px;height:32px;background:${o(athlete.full_name)};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-family:var(--font-display);font-weight:700;font-size:0.85rem;color:#000;border-radius:6px;">${null !== athlete.jersey_number ? Utils.escapeHtml(String(athlete.jersey_number)) : Utils.initials(athlete.full_name)}</div><span style="font-weight:600;">${Utils.escapeHtml(athlete.full_name)}</span></div></td><td style="font-size:12px;color:var(--color-text-muted);">${Utils.escapeHtml(i(athlete.category, athlete.team_name))}</td>${0 === metricKeys.length ? '<td style="text-align:center;color:var(--color-text-muted);font-size:12px;">Nessuna metrica</td>' : metricKeys.map((tk) => `<td style="text-align:center;">${renderMetricCell(athlete.metrics, tk)}</td>`).join("")}</tr>`,
                )
                .join("");
              const avgCells =
                0 === metricKeys.length
