...
const Transport = (() => {
  ...
  function m() {
    Utils.qsa("[data-event-id]").forEach(
      (e) =>
        e.addEventListener("click", () => b(e.dataset.eventId), {
          signal: t.signal,
        }),
      { signal: t.signal },
    );
  ...
  async function b(e) {
    ...
    try {
      const [a, i, r] = await Promise.all([
        ...
        s = App.getUser(),
        o = ["admin", "manager", "operator"].includes(s?.role),
      ...
      ? ((e.innerHTML = i
          ...
          e.addEventListener(
            ...
            (t) => {
              ...
              const n = e.dataset.athleteId,
                a = i.find((t) => t.id === n);
              if (!a) return;
              const r = o.findIndex((t) => t.id === n);
              (r >= 0 ? o.splice(r, 1) : o.push({ ...a }), y(), w());
            },
            { signal: t.signal },
          );
        }),
        e.querySelectorAll(".nt-addr-input").forEach((e) => {
          (e.addEventListener(
            ...
            k(() =>
              z(e, ({ lat: t, lng: n, address: a }) => {
                const r = e.dataset.addrFor;
                d.set(a, { lat: t, lng: n });
                const s = o.find((t) => t.id === r),
                  l = i.find((t) => t.id === r);
                (s && (s.residence_address = a),
  ...
  function z(t, e) {
    ...
    n.addListener("place_changed", () => {
      ...
      const i = a.geometry.location.lat(),
        ...
        s = a.formatted_address || t.value;
      ((t.value = s), e && e({ lat: i, lng: r, address: s, place: a }));
    });
  }
  function E() {
    let t = null,
      ...
      document
        ...
        ?.addEventListener("click", async () => {
          const i = document.getElementById("gym-name").value.trim(),
            s = document.getElementById("gym-address").value.trim(),
            o = document.getElementById("gym-error");
          if (!i)
            return (
              (o.textContent = "Nome obbligatorio"),
              void o.classList.remove("hidden")
            );
          const l = document.getElementById("gym-save");
          ((l.disabled = !0), (l.textContent = "Salvataggio..."));
          try {
            const o = await Store.api("createGym", "transport", {
              name: i,
              address: s || null,
              lat: t,
              lng: e,
            }),
              l = { id: o.id, name: i, address: s, lat: t, lng: e };
            n.push(l);
  ...
  async function $(t, e, n, a = !0) {
    return new Promise((i, r) => {
      k(() => {
        ...
        const s = new google.maps.DirectionsService(),
          o = {
            origin: t,
            destination: e,
  ...
  async function _() {
    ...
    try {
      ...
      const i = o.map((t) => t.residence_address);
      let s = null;
      try {
        s = await $(e, n, i, !0);
      } catch (t) {
      ...
      for (let t = 0; t < d.legs.length; t++) {
        ...
        ((c += n),
          (g += a + i),
          m.push(a + i),
          b.push({ durata: e.duration.text, distanza: e.distance.text }),
          u.push({
  ...
  function U(e, n, a, i) {
    const s = document.getElementById("nt-results");
    if (
          ...
          (function (t) {
            ...
            (t
              ...
              n.forEach((a, i) => {
                const r = 0 === i,
                  s = i === n.length - 1,
                  o = r ? "#00e5ff" : s ? "#00E676" : "#FF00FF",
                  l = `<div style="width:32px;height:32px;border-radius:50%;background:${o};color:${r || s ? "#000" : "#fff"};display:flex;align-items:center;justify-content:center;font-weight:800;font-size:13px;border:2px solid rgba(255,255,255,0.8);box-shadow:0 2px 8px rgba(0,0,0,0.6);">${r ? "🚐" : s ? "🏟" : String(i)}</div>`,
                  d = t.divIcon({
                    html: l,
                    className: "",
                    iconSize: [32, 32],
                ...
                t.marker([a.lat, a.lng], { icon: d })
                  .addTo(e)
                  .bindPopup(
                    `<b style="color:${o}">${r ? "Partenza" : s ? "Palestra" : "Tappa " + i}</b><br><small>${p}</small>`,
                  );
              }),
                    ...
                    if (
                      ...
                      try {
                        const t =
                          ...
                          s = (await $(t, e, i, !1)).routes[0];
                        let o = 0,
                          d = 0;
                        const p = [],
                          c = [];
                        c.push({
                          lat: s.legs[0].start_location.lat(),
                          lng: s.legs[0].start_location.lng(),
                          label: t,
                        });
                        for (let t = 0; t < s.legs.length; t++) {
                          const e = s.legs[t],
                            n = e.distance.value,
                            a = e.duration.value,
                            i = t > 0 && t < s.legs.length - 1 ? 180 : 0;
                          ((o += n),
                            (d += a + i),
                            p.push(a + i),
                            c.push({
                        ...
                        const g = {
                          ...
                          distanza: (o / 1e3).toFixed(1) + " km",
                          tappe: n.length,
                        },
                          [m, u] = a.split(":").map(Number),
                          b = new Date();
                        b.setHours(m, u, 0, 0);
                        const f = new Date(b.getTime() - 1e3 * d),
                          x = [];
                        let v = new Date(f);
                        (x.push({
                          tipo: "partenza",
  ...
  async function L() {
    if (!l || !r || !s) return void UI.toast("Dati incompleti", "error");
