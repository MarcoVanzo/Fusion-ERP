    ...
    i.length
      ? ((e.innerHTML = i
          .map((t) => {
            ...
            return `\\n        <div class="nt-athlete-card ${i ? "selected" : ""}" data-athlete-id="${Utils.escapeHtml(t.id)}" style="background:rgba(255,255,255,0.02);border:1px solid ${i ? '#00e676' : 'rgba(255,255,255,0.1)'};border-radius:12px;padding:16px;display:flex;align-items:center;gap:16px;cursor:pointer;transition:all 0.2s;">\\n          <div style="width:40px;height:40px;border-radius:50%;background:rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:${i ? '#fff' : 'rgba(255,255,255,0.5)'};">${e}</div>\\n          <div style="flex:1; min-width:0;">\\n            <div style="font-weight:600;font-size:14px;color:${i ? '#fff' : 'rgba(255,255,255,0.7)'};">${Utils.escapeHtml(t.full_name)}</div>\\n            <div style="font-size:12px;color:${a ? 'rgba(255,255,255,0.4)' : '#ff5252'};margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">\\n              ${a ? '<i class="ph ph-map-pin"></i> ' + Utils.escapeHtml(t.residence_address) : '<i class="ph ph-warning"></i> Indirizzo mancante'}\\n            </div>\\n            ${!a && i ? `<input class="nt-addr-input form-input" style="margin-top:8px;padding:6px 10px;font-size:12px;height:auto;" type="text" data-addr-for="${Utils.escapeHtml(t.id)}" placeholder="Inserisci indirizzo..." onclick="event.stopPropagation()">` : ""}\\n          </div>\\n          <div style="color:${i ? '#00e676' : 'rgba(255,255,255,0.2)'};font-size:24px;">\\n            <i class="ph ${i ? 'ph-check-circle-fill' : 'ph-circle'}"></i>\\n          </div>\\n        </div>`;
          })
          .join("")),
