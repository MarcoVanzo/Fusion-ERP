...
const Newsletter = (() => {
    ...
    function renderSubscribersTable() {
        ...
        const isAdmin = ["admin", "manager"].includes(App.getUser()?.role);
        const statusLabel = { active: 'Attivo', unsubscribed: 'Disiscriito', bounced: 'Bounce', unconfirmed: 'Non confermato', junk: 'Spam' };
        const statusColor = { active: 'var(--color-success)', unsubscribed: 'var(--color-text-muted)', bounced: 'var(--color-pink)', unconfirmed: '#f59e0b', junk: 'var(--color-pink)' };
    ...
    function bindEvents(isAdmin) {
        ...
        document.getElementById('btn-nl-loadmore')?.addEventListener('click', async () => {
            ...
            if (_nextCursor) {
                ...
                if (newBtn) { newBtn.disabled = false; newBtn.innerHTML = '<i class="ph ph-arrow-down"></i> Carica altri'; }
            } else {
        ...
        if (isAdmin) {
            // Add subscriber
            document.getElementById('btn-nl-add-subscriber')?.addEventListener('click', () => openAddModal(), sig());

            // Manage groups
            document.getElementById('btn-nl-groups')?.addEventListener('click', () => openGroupsModal(), sig());
    ...
    function openAddModal() {
        const groupsOptions = _groups.map(g =>
        ...
        document.getElementById('nl-sub-save')?.addEventListener('click', async () => {
            ...
            const groups  = [...document.querySelectorAll('.nl-group-check:checked')].map(c => c.value);
            ...
            saveBtn.disabled = true; saveBtn.textContent = 'Aggiunta...';

            try {
                await Store.api('upsertSubscriber', 'newsletter', { email, name: name || null, last_name: last || null, groups });
                _subscribers = [];
                _nextCursor = null;
                await loadSubscribers(false);
                ...
                errEl.classList.remove('hidden');
                saveBtn.disabled = false; saveBtn.textContent = 'AGGIUNGI';
            }
        });
    }
    ...
    function openGroupsModal() {
        function renderGroupList() {
            if (_groups.length === 0) return `<p style="text-align:center;color:var(--color-text-muted);padding:20px 0;font-size:14px;">Nessun gruppo creato</p>`;
        ...
        document.getElementById('nl-group-create')?.addEventListener('click', async () => {
            ...
            if (!name) { errEl.textContent = 'Il nome è obbligatorio'; errEl.classList.remove('hidden'); return; }
            const btn = document.getElementById('nl-group-create');
            btn.disabled = true; btn.textContent = 'Creazione...';
            try {
                const newGroup = await Store.api('createGroup', 'newsletter', { name });
                if (newGroup?.id) _groups.push(newGroup);
                else _groups = await Store.get('listGroups', 'newsletter').catch(() => _groups);
                const list = document.getElementById('nl-groups-list');
                if (list) list.innerHTML = renderGroupList();
                rebindDeleteBtns();
            ...
            finally { btn.disabled = false; btn.innerHTML = '<i class="ph ph-plus"></i> Crea'; }
        });
    }
    ...
    return {
        async init() {
            ...
            try {
                ...
                const config = await Store.get('getConfig', 'newsletter').catch(() => ({ configured: false }));
