...
const Societa = (() => {
  ...
  function p(e) {
    ...
    ((e.innerHTML = `\n            <div>\n                ${t ? '<div style="display:flex;justify-content:flex-end;margin-bottom:var(--sp-3)">\n                    <button class="btn btn-primary btn-sm" id="soc-add-role" type="button"><i class="ph ph-plus"></i> NUOVO RUOLO</button>\n                </div>' : ""}\n                <div class="soc-tree" id="soc-tree">\n                    ${
      0 === o.length
        ? Utils.emptyState(
            "Nessun ruolo",
            "Aggiungi il primo ruolo con il pulsante in alto.",
          )
        : a
            .map(function e(a) {
              const l = ((n = a.id), o.filter((e) => e.parent_role_id === n));
              var n;
              return `\n                <div style="margin-bottom:var(--sp-2)">\n                    <div class="soc-tree-node" draggable="true" data-role-id="${Utils.escapeHtml(a.id)}">\n                        <i class="ph ph-dots-six-vertical soc-tree-drag-handle"></i>\n                        <div style="flex:1">\n                            <div class="soc-tree-node-name">${Utils.escapeHtml(a.name)}</div>\n                            ${a.description ? `<div class="soc-tree-node-desc">${Utils.escapeHtml(a.description)}</div>` : ""}\n                        </div>\n                        ${t ? `\n                        <button class="btn btn-ghost btn-sm" data-edit-role="${Utils.escapeHtml(a.id)}" type="button" title="Modifica"><i class="ph ph-pencil-simple"></i></button>\n                        <button class="btn btn-ghost btn-sm" data-del-role="${Utils.escapeHtml(a.id)}" type="button" title="Elimina" style="color:var(--color-pink)"><i class="ph ph-trash"></i></button>\n                        ` : ""}\n                    </div>\n                    ${l.length ? `<div class="soc-tree-level">${l.map(e).join("")}</div>` : ""}\n                </div>`;
            })
            .join("")
    }\n                </div>\n            </div>`),
  ...
  function m(e) {
    const t = !!e,
      ...
      l = UI.modal({
        ...
        footer: `\n                <button class="btn btn-ghost btn-sm" id="rl-cancel" type="button">Annulla</button>\n                <button class="btn btn-primary btn-sm" id="rl-save" type="button">${t ? "SALVA" : "CREA RUOLO"}</button>`,
      });
    (document
            ...
            .map(
              (e) =>
                `\n                            <div class="soc-doc-card">\n                                <div style="display:flex;align-items:flex-start;justify-content:space-between">\n                                    <i class="ph ph-${o[e.category] || "file"} soc-doc-icon"></i>\n                                    ${t ? `<button class="btn btn-ghost btn-sm" data-del-doc="${Utils.escapeHtml(e.id)}" type="button" style="color:var(--color-pink)"><i class="ph ph-trash"></i></button>` : ""}\n                                </div>\n                                <div class="soc-doc-name" title="${Utils.escapeHtml(e.file_name)}">${Utils.escapeHtml(e.file_name)}</div>\n                                <div class="soc-doc-meta">${Utils.escapeHtml(a[e.category] || e.category)}</div>\n                                ${e.expiry_date ? `<div class="soc-doc-meta">Scad. ${Utils.escapeHtml(Utils.formatDate ? Utils.formatDate(e.expiry_date) : e.expiry_date)}</div>` : ""}\n                                <div style="margin-top:4px">${(function (
                  ...
                  e.expiry_date,
                )}</div>\n                                <a href="api/?module=societa&action=downloadDocument&docId=${Utils.escapeHtml(e.id)}" target="_blank" class="btn btn-default btn-sm" style="margin-top:var(--sp-1)">\n                                    <i class="ph ph-download-simple"></i> Scarica\n                                </a>\n                            </div>`,
            )
            .join("")
    }\n                </div>\n            </div>`),
    ...
    ((e.innerHTML = `
            <div>
                <div style="display:flex;align-items:center;justify-content:space-between;gap:var(--sp-2);margin-bottom:var(--sp-3);flex-wrap:wrap">
                    ...
                    ${t ? '<button class="btn btn-primary btn-sm" id="soc-add-deadline" type="button"><i class="ph ph-plus"></i> NUOVA SCADENZA</button>' : ""}
                </div>
                <div style="display:flex;flex-direction:column;gap:var(--sp-2)" id="soc-deadlines-list">
                    ${
                      0 === s.length
                        ? Utils.emptyState(
                            "Nessuna scadenza",
                            "Aggiungi la prima scadenza federale.",
                          )
                        : s
                            .map((e) => {
                              return `
                            <div class="card" style="padding:var(--sp-3);display:flex;align-items:center;gap:var(--sp-3);flex-wrap:wrap" data-dl-status="${Utils.escapeHtml(e.status)}">
                                ...
                                <div style="display:flex;gap:4px">
                                    <button class="btn btn-ghost btn-sm" data-edit-dl="${Utils.escapeHtml(e.id)}" type="button"><i class="ph ph-pencil-simple"></i></button>
                                    <button class="btn btn-ghost btn-sm" data-del-dl="${Utils.escapeHtml(e.id)}" type="button" style="color:var(--color-pink)"><i class="ph ph-trash"></i></button>
                                </div>`
                                    : ""
                                }
                            </div>`;
      ...
      a = UI.modal({
        ...
        body: `
                <div class="form-group">
                    ...
                    <input id="dl-title" class="form-input" type="text" value="${Utils.escapeHtml(e?.title || "")}" placeholder="es. Rinnovo affiliazione FIPAV">
                </div>
                <div class="form-grid">
                    <div class="form-group">
                        <label class="form-label" for="dl-due">Data Scadenza *</label>
                        <input id="dl-due" class="form-input" type="date" value="${e?.due_date?.substring(0, 10) || ""}"/>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="dl-status">Stato</label>
                        <select id="dl-status" class="form-select">
                            ${["aperto", "completato", "scaduto", "annullato"].map((t) => `<option value="${t}" ${e?.status === t ? "selected" : ""} >${t.charAt(0).toUpperCase() + t.slice(1)}</option>`).join("")}
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="dl-cat">Categoria</label>
                    <input id="dl-cat" class="form-input" type="text" value="${Utils.escapeHtml(e?.category || "")}" placeholder="es. Federale, Fiscale…">
                </div>
                <div class="form-group">
                    <label class="form-label" for="dl-notes">Note</label>
                    <textarea id="dl-notes" class="form-input" rows="2">${Utils.escapeHtml(e?.notes || "")}</textarea>
                </div>
                <div id="dl-error" class="form-error hidden"></div>`,
        footer: `
                <button class="btn btn-ghost btn-sm" id="dl-cancel" type="button">Annulla</button>
                <button class="btn btn-primary btn-sm" id="dl-save" type="button">${t ? "SALVA" : "CREA"}</button>`,
      });
    (document
  ...
  function renderTitoli(el) {
    ...
    el.innerHTML = `
            <div>
                ${isAdmin ? '<div style="display:flex;justify-content:flex-end;margin-bottom:var(--sp-3)"><button class="btn btn-primary btn-sm" id="soc-add-titolo" type="button"><i class="ph ph-plus"></i> AGGIUNGI TITOLO</button></div>' : ""}
                <div class="table-wrapper" style="overflow-x:auto">
                    <table class="data-table" style="width:100%;border-collapse:collapse;font-size:14px">
                        <thead><tr>
                            <th style="text-align:left;padding:10px 12px;border-bottom:1px solid var(--color-border);font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-text-muted)">Stagione</th>
                            <th style="text-align:left;padding:10px 12px;border-bottom:1px solid var(--color-border);font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-text-muted)">Campionato</th>
                            ...
                            <th style="text-align:left;padding:10px 12px;border-bottom:1px solid var(--color-border);font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-text-muted)">Piazzamento</th>
                            <th style="text-align:left;padding:10px 12px;border-bottom:1px solid var(--color-border);font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-text-muted)">Finali Naz.</th>
                            ${isAdmin ? '<th style="padding:10px 12px;border-bottom:1px solid var(--color-border)"></th>' : ""}
                        </tr></thead>
                        <tbody id="soc-titoli-tbody">
                            ${
                              0 === titoli.length
                                ? `<tr><td colspan="${isAdmin ? 6 : 5}" style="text-align:center;padding:var(--sp-4);color:var(--color-text-muted)">Nessun titolo registrato</td></tr>`
                                : titoli
                                    .map(
                                      (row) => `
                                <tr>
                                    ...
                                    ${
                                      isAdmin
                                        ...
                                        <button class="btn btn-ghost btn-sm" data-edit-titolo="${Utils.escapeHtml(row.id)}" type="button"><i class="ph ph-pencil-simple"></i></button>
                                        <button class="btn btn-ghost btn-sm" data-del-titolo="${Utils.escapeHtml(row.id)}" type="button" style="color:var(--color-pink)"><i class="ph ph-trash"></i></button>
                                    </td>`
                                        : ""
                                    }
                                </tr>`,
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select id="tt-campionato" class="form-select">
                        ${["provinciale", "regionale", "nazionale"].map((v) => `<option value="${v}" ${titolo?.campionato === v ? "selected" : ""} >${v.charAt(0).toUpperCase() + v.slice(1)}</option>`).join("")}
                    </select>
                </div>
            </div>
            <div class="form-group">
                <label class="form-label" for="tt-categoria">Categoria *</label>
                <input id="tt-categoria" class="form-input" type="text" value="${Utils.escapeHtml(titolo?.categoria || "")}" placeholder="es. Under 18 Femminile">
            </div>
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label" for="tt-piazzamento">Piazzamento *</label>
                    <select id="tt-piazzamento" class="form-select">
                        <option value="1" ${titolo?.piazzamento == 1 ? "selected" : ""}>🥇 1° Posto</option>
                        <option value="2" ${titolo?.piazzamento == 2 ? "selected" : ""}>🥈 2° Posto</option>
                        <option value="3" ${titolo?.piazzamento == 3 ? "selected" : ""}>🥉 3° Posto</option>
                    </select>
                </div>
                <div class="form-group" style="display:flex;align-items:center;gap:var(--sp-2);padding-top:var(--sp-4)">
                    <input type="checkbox" id="tt-finali" style="width:18px;height:18px;cursor:pointer" ${parseInt(titolo?.finali_nazionali) ? "checked" : ""}>
                    <label for="tt-finali" class="form-label" style="margin:0;cursor:pointer">Partecipazione Finali Nazionali</label>
                </div>
            </div>
            <div class="form-group">
                <label class="form-label" for="tt-note">Note</label>
                <textarea id="tt-note" class="form-input" rows="2" placeholder="Informazioni aggiuntive...">${Utils.escapeHtml(titolo?.note || "")}</textarea>
            </div>
            <div id="tt-error" class="form-error hidden"></div>`,
      footer: `
            <button class="btn btn-ghost btn-sm" id="tt-cancel" type="button">Annulla</button>
            <button class="btn btn-primary btn-sm" id="tt-save" type="button">${isEdit ? "SALVA" : "CREA TITOLO"}</button>`,
    });
    document
  ...
  function spRender(el) {
    const isAdmin = ["admin", "manager"].includes(App.getUser()?.role);
    el.innerHTML = `<div>${isAdmin ? '<div style="display:flex;justify-content:flex-end;margin-bottom:var(--sp-3)"><button class="btn btn-primary btn-sm" id="soc-add-sponsor" type="button"><i class="ph ph-plus"></i> NUOVO SPONSOR</button></div>' : ""}${sp_list.length === 0 ? Utils.emptyState("Nessuno sponsor", "Aggiungi il primo sponsor con il pulsante in alto.") : '<div class="soc-sponsor-grid">' + sp_list.map((sp) => `<div class="soc-sponsor-card${sp.is_active ? "" : " soc-sponsor-inactive"}"><div class="soc-sponsor-card-header">${sp.logo_path ? `<img src="${Utils.escapeHtml(sp.logo_path)}" alt="Logo" class="soc-sponsor-logo">` : '<div class="soc-sponsor-logo-placeholder"><i class="ph ph-image"></i></div>'}${isAdmin ? `<div style="display:flex;gap:4px"><button class="btn btn-ghost btn-sm" data-sp-logo="${Utils.escapeHtml(sp.id)}" type="button" title="Carica logo"><i class="ph ph-camera"></i></button><button class="btn btn-ghost btn-sm" data-sp-edit="${Utils.escapeHtml(sp.id)}" type="button" title="Modifica"><i class="ph ph-pencil-simple"></i></button><button class="btn btn-ghost btn-sm" data-sp-del="${Utils.escapeHtml(sp.id)}" type="button" style="color:var(--color-pink)" title="Elimina"><i class="ph ph-trash"></i></button></div>` : ""}</div><div class="soc-sponsor-name">${Utils.escapeHtml(sp.name)}</div><div style="font-size:11px;color:var(--color-text-muted);margin-top:2px;text-transform:uppercase;letter-spacing:0.05em">${Utils.escapeHtml(sp.tipo || "Sponsor")}</div>${sp.description ? `<div class="soc-sponsor-desc">${Utils.escapeHtml(sp.description)}</div>` : ""}<div class="soc-sponsor-links">${sp.website_url ? `<a href="${Utils.escapeHtml(sp.website_url)}" target="_blank" class="soc-sponsor-link" title="Sito web"><i class="ph ph-globe-simple"></i></a>` : ""}${sp.instagram_url ? `<a href="${Utils.escapeHtml(sp.instagram_url)}" target="_blank" class="soc-sponsor-link" title="Instagram"><i class="ph ph-instagram-logo"></i></a>` : ""}${sp.facebook_url ? `<a href="${Utils.escapeHtml(sp.facebook_url)}" target="_blank" class="soc-sponsor-link" title="Facebook"><i class="ph ph-facebook-logo"></i></a>` : ""}${sp.linkedin_url ? `<a href="${Utils.escapeHtml(sp.linkedin_url)}" target="_blank" class="soc-sponsor-link" title="LinkedIn"><i class="ph ph-linkedin-logo"></i></a>` : ""}${sp.tiktok_url ? `<a href="${Utils.escapeHtml(sp.tiktok_url)}" target="_blank" class="soc-sponsor-link" title="TikTok"><i class="ph ph-tiktok-logo"></i></a>` : ""}</div></div>`).join("") + "</div>"}</div>`;
    document
      ...
      btn.addEventListener(
        ...
        async () => {
          ...
          const m = UI.modal({
            ...
            footer:
              '<button class="btn btn-ghost btn-sm" id="sp-del-cancel" type="button">Annulla</button><button class="btn btn-primary btn-sm" id="sp-del-ok" type="button" style="background:var(--color-pink)">ELIMINA</button>',
          });
          document
  ...
  function renderNewsletterTab(el) {
    el.innerHTML = `
            <div style="max-width: 760px;">
                <div class="card" style="padding:var(--sp-4);margin-bottom:var(--sp-3)">
                    <p class="section-label" style="margin-bottom:var(--sp-3)">Impostazioni Newsletter</p>
                    ...
                    <div class="form-group">
                        <label class="form-label" for="newsletter-status">Stato Newsletter</label>
                    ...
                    <div class="form-group">
                        <label class="form-label" for="newsletter-email">Email Mittente</label>
                        <input id="newsletter-email" class="form-input" type="email" placeholder="newsletter@tua-societa.it">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="newsletter-template">Template Predefinito</label>
                        <textarea id="newsletter-template" class="form-input" rows="5" placeholder="HTML del template..."></textarea>
                    </div>
                    <div style="display:flex;justify-content:flex-end;margin-top:var(--sp-2)">
                        <div id="newsletter-err" class="form-error hidden"></div>
                        <button class="btn btn-primary" id="newsletter-save" type="button"><i class="ph ph-floppy-disk"></i> SALVA IMPOSTAZIONI</button>
                    </div>
                </div>
            </div>
        `;
  ...
  function renderForesteria(el) {
    ...
    el.innerHTML = `<div style="display:flex;flex-direction:column;gap:var(--sp-4)">
      <div class="card" style="padding:var(--sp-4)">
        <p class="section-label" style="margin-bottom:var(--sp-3)"><i class="ph ph-house-line"></i> La Foresteria</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-3)">
          <div>
            ...
            <div class="form-group">
              ...
              <input class="form-input" type="text" value="${Utils.escapeHtml(info.address || "Via Bazzera 16, 30030 Martellago (VE)")}" readonly style="background:var(--color-surface-elevated)">
            </div>
            ${isAdmin ? '<button class="btn btn-primary btn-sm" id="for-save-desc" type="button"><i class="ph ph-floppy-disk"></i> SALVA DESCRIZIONE</button>' : ""}
          </div>
          <div>
            <iframe src="https://maps.google.com/maps?q=Via+Bazzera+16+Martellago+VE+Italy&output=embed&z=15" width="100%" height="220" style="border:0;border-radius:12px" allowfullscreen loading="lazy"></iframe>
            <p style="font-size:11px;color:var(--color-text-muted);margin-top:4px;text-align:center"><i class="ph ph-map-pin"></i> Via Bazzera 16, 30030 Martellago (VE)</p>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="padding:var(--sp-4)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--sp-3);flex-wrap:wrap;gap:var(--sp-2)">
          <p class="section-label" style="margin:0"><i class="ph ph-images"></i> Foto, Video &amp; YouTube</p>
          ${isAdmin ? `<div style="display:flex;gap:var(--sp-2);flex-wrap:wrap"><label class="btn btn-primary btn-sm" style="cursor:pointer"><i class="ph ph-upload-simple"></i> CARICA MEDIA<input type="file" id="for-media-input" accept="image/*,video/*" style="display:none" multiple></label><button class="btn btn-default btn-sm" id="for-add-youtube" type="button"><i class="ph ph-youtube-logo"></i> AGGIUNGI LINK YOUTUBE</button></div>` : ""}
        </div>
        ${media.length === 0 ? Utils.emptyState("Nessun media", "Carica foto, video o aggiungi un link YouTube della foresteria.") : ""}
        ${
          photos.length > 0
            ? `<p style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--color-text-muted);margin-bottom:var(--sp-2)">Foto (${photos.length})</p><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:var(--sp-2);margin-bottom:var(--sp-3)">${photos
                .map(
                  (
                    ph,
                  ) => `<div style="position:relative;border-radius:10px;overflow:hidden;aspect-ratio:1">
          <img src="${Utils.escapeHtml(ph.file_path)}" alt="${Utils.escapeHtml(ph.title || "foto")}" style="width:100%;height:100%;object-fit:cover">
          ${isAdmin ? `<button data-del-media="${Utils.escapeHtml(ph.id)}" class="btn btn-ghost btn-sm" style="position:absolute;top:4px;right:4px;background:rgba(0,0,0,.7);color:#fff;padding:4px" type="button"><i class="ph ph-trash"></i></button>` : ""}
        </div>`,
                )
                .join("")}</div>`
            : ""
        ...
        ${
          videos.length > 0
            ? `<p style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--color-text-muted);margin-bottom:var(--sp-2)">Video (${videos.length})</p><div style="display:flex;flex-direction:column;gap:var(--sp-2);margin-bottom:var(--sp-3)">${videos
                .map(
                  (
                    v,
          ...
          <div style="flex:1"><div style="font-size:13px;font-weight:600">${Utils.escapeHtml(v.title || "Video")}</div><div style="font-size:11px;color:var(--color-text-muted)">${Utils.escapeHtml(v.file_path?.split("/").pop() || "")} </div></div>
          <a href="${Utils.escapeHtml(v.file_path)}" target="_blank" class="btn btn-ghost btn-sm"><i class="ph ph-play"></i></a>
          ${isAdmin ? `<button data-del-media="${Utils.escapeHtml(v.id)}" class="btn btn-ghost btn-sm" style="color:var(--color-pink)" type="button"><i class="ph ph-trash"></i></button>` : ""}
        </div>`,
                )
                .join("")}</div>`
            : ""
        ...
        ${(() => {
          ...
          if (!youtubes.length) return "";
          const getYTId = (url) => {
            const m = url.match(/[?&]v=([a-zA-Z0-9_-]{11})/);
            return m ? m[1] : null;
          };
          return `<p style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--color-text-muted);margin-bottom:var(--sp-2)">YouTube (${youtubes.length})</p><div style="display:flex;flex-direction:column;gap:var(--sp-3)">${youtubes
            .map((yt) => {
              const vid = getYTId(yt.file_path);
              return `<div style="background:var(--color-surface-elevated);border-radius:12px;overflow:hidden">
          <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden">${vid ? `<iframe src="https://www.youtube.com/embed/${vid}" title="${Utils.escapeHtml(yt.title || "Video YouTube")}" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0" allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture" allowfullscreen loading="lazy"></iframe>` : `<div style="position:absolute;top:0;left:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:var(--color-text-muted)">Video non disponibile</div>`}</div>
          <div style="display:flex;align-items:center;gap:var(--sp-2);padding:var(--sp-2) var(--sp-3)">
            ...
            <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${Utils.escapeHtml(yt.title || "Video YouTube")}</div><a href="${Utils.escapeHtml(yt.file_path)}" target="_blank" style="font-size:11px;color:var(--color-text-muted)">${Utils.escapeHtml(yt.file_path)}</a></div>
            ${isAdmin ? `<button data-del-media="${Utils.escapeHtml(yt.id)}" class="btn btn-ghost btn-sm" style="color:var(--color-pink)" type="button"><i class="ph ph-trash"></i></button>` : ""}
          </div>
        </div>`;
            })
            .join("")}</div>`;
    ...
    document.getElementById("for-add-youtube")?.addEventListener(
      ...
      () => {
        const modal = UI.modal({
          ...
          footer:
            '<button class="btn btn-ghost btn-sm" id="yt-cancel" type="button">Annulla</button><button class="btn btn-primary btn-sm" id="yt-save" type="button"><i class="ph ph-youtube-logo"></i> AGGIUNGI</button>',
        });
        document
