...
const Finance = (() => {
  ...
  async function o() {
    ...
    if (e) {
      ...
      try {
          ...
          (function (e) {
            ...
            ((e.innerHTML = `\n        <div class="transport-dashboard" style="min-height:100vh;">\n            <div class="dash-top-bar" style="border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 24px; margin-bottom: 24px;">
                <div style="display:flex;justify-content:space-between;width:100%;align-items:center;flex-wrap:wrap;gap:12px;">
                    \n                    <div><h1 class="dash-title" style="display:flex;align-items:center;gap:10px;">\n                        <i class="ph ph-calculator" style="color:var(--accent-pink);"></i>\n                        Contabilità\n                    </h1></div>\n                    <div style="display:flex;gap:8px;">\n                        <button class="btn-dash" id="btn-view-entries" type="button">\n                            <i class="ph ph-list"></i> Prima Nota\n                        </button>\n                        <button class="btn-dash" id="btn-view-invoices" type="button">\n                            <i class="ph ph-file-invoice"></i> Fatture Elettroniche\n                        </button>\n                        <button class="btn-dash" id="btn-view-accounts" type="button">\n                            <i class="ph ph-tree-structure"></i> Piano dei Conti\n                        </button>\n                        <button class="btn-dash" id="btn-rendiconto" type="button">\n                            <i class="ph ph-file-text"></i> Rendiconto\n                        </button>\n                        <button class="btn-dash" id="btn-74ter" type="button">\n                            <i class="ph ph-intersect"></i> 74-ter\n                        </button>\n                        <button class="btn-dash pink" id="btn-new-entry" type="button">\n                            <i class="ph ph-plus"></i> Nuova Registrazione\n                        </button>\n                    
                </div>
              ...
              const t = Math.max(
                ...n.map((n) =>
                  Math.max(
                    parseFloat(n.income) || 0,
                    parseFloat(n.expenses) || 0,
                  ),
                ),
                1,
              );
              return `\n        <div class="dash-card" style="padding:var(--sp-4);margin-bottom:var(--sp-4);">\n            <h3 style="margin-bottom:var(--sp-2);display:flex;align-items:center;gap:8px;">\n                <i class="ph ph-chart-bar" style="color:var(--accent-pink);"></i>\n                Andamento Mensile\n            </h3>\n            <div class="finance-chart">\n                ${n
                .map((n) => {
                  const e = parseFloat(n.income) || 0,
                    a = parseFloat(n.expenses) || 0,
                    i = Math.max(2, (e / t) * 100),
                    s = Math.max(2, (a / t) * 100),
                    l = n.month ? n.month.split("-")[1] : "",
                    r =
                      [
                        "",
                        "Gen",
              ...
              .join("")}\n        </div > `),
                  ...
                  t.querySelector("#btn-rendiconto")?.addEventListener(
                    "click",
                    () =>
                      (async function (t) {
                        t.innerHTML = UI.skeletonPage();
                ...
                (n) =>
                  `\n            <div class="finance-rendiconto-section">\n                <h3 class="finance-rendiconto-title">${Utils.escapeHtml(n.label)}</h3>\n                ${
                    (n.accounts || [])
                      ...
                      .join("") ||
                    '<p style="color:var(--text-muted);font-size:13px;padding:8px 0;">Nessun movimento</p>'
              ...
              .join(
                "",
                  ...
                  t
                    .querySelector("#btn-74ter")
                    ?.addEventListener("click", () => m(t), {
                      signal: n.signal,
                    }),
                  t.querySelector(".finance-entries-list")?.addEventListener(
                    "click",
                    (n) => {
        ...
        (console.error("[Finance] Init error:", n),
  ...
  function c(n) {
    const t = (n.category || "").match(
        /quote|iscrizi|sponsor|contribut|donaz|event|5.*mille/i,
      ),
      e = t ? "arrow-circle-down" : "arrow-circle-up",
      a = t ? "#10b981" : "#ef4444",
      i = s[n.category] || n.category || "—",
      l = n.entry_date
        ? new Date(n.entry_date).toLocaleDateString("it-IT", {
            day: "2-digit",
            month: "short",
          })
        : "";
  ...
  async function d() {
    ...
    const t = Object.entries(s)
        .map(([n, t]) => `<option value="${n}">${Utils.escapeHtml(t)}</option>`)
        .join(""),
      e = a
        .map(
          (n) =>
            `<option value="${Utils.escapeHtml(n.id)}">[${Utils.escapeHtml(n.code)}] ${Utils.escapeHtml(n.name)} (${n.type})</option>`,
        )
        .join("");
    ...
    const i = UI.modal({ title: "Nuova Registrazione", body: n, size: "lg" });
    n.querySelector("#entry-form").addEventListener("submit", async (t) => {
      ...
      if (!s || l <= 0)
        return (
          (a.textContent = "Descrizione e importo sono obbligatori"),
      ...
      try {
        ...
        ((a.textContent = n.message || "Errore"),
          a.classList.remove("hidden"),
          (e.disabled = !1),
          (e.innerHTML = '<i class="ph ph-plus"></i> Registra'));
      }
    });
  }
  async function p(t) {
    ...
    try {
      ...
      ((e = a?.entries || []),
        (function (t, a) {
          ((t.innerHTML = `\n        <div class="transport-dashboard" style="min-height:100vh;">\n            <div class="dash-top-bar" style="border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 24px; margin-bottom: 24px;">
                <div style="display:flex;justify-content:space-between;width:100%;align-items:center;flex-wrap:wrap;gap:12px;">
                    \n                    <div><h1 class="dash-title" style="display:flex;align-items:center;gap:10px;">\n                        <i class="ph ph-list" style="color:var(--accent-pink);"></i> Prima Nota\n                    </h1></div>\n                    <div style="display:flex;gap:8px;">\n                        <button class="btn-dash" id="btn-back-dash">\n                            <i class="ph ph-arrow-left"></i> Dashboard\n                        </button>\n                        <button class="btn-dash pink" id="btn-new-entry">\n                            <i class="ph ph-plus"></i> Nuova\n                        </button>\n                    
                </div>
            ...
            t
              .querySelector("#btn-back-dash")
              ?.addEventListener("click", () => o(), { signal: n.signal }),
            t
              .querySelector("#btn-new-entry")
              ?.addEventListener("click", () => d(), { signal: n.signal }));
        })(t, a));
    } catch (n) {
      t.innerHTML = Utils.emptyState("Errore", n.message, "Riprova", null, () =>
        p(t),
      );
    }
  }
  ...
  function loadChartJsFex() {
    return (
      _fexChartJsLoaded
        ...
        : _fexChartJsPromise
          ? _fexChartJsPromise
          : (_fexChartJsPromise = new Promise((e, r) => {
              const t = document.createElement("script");
              ((t.src = "https://cdn.jsdelivr.net/npm/chart.js"),
                (t.onload = () => {
    ...
    try {
      ...
      t.querySelector("#for-add-exp")?.addEventListener("click", () => {
        const modal = UI.modal({
          ...
          footer:
            '<button class="btn btn-ghost btn-sm" id="fex-cancel" type="button">Annulla</button><button class="btn btn-primary btn-sm" id="fex-save" type="button">AGGIUNGI</button>',
        });
        document
        ...
        document
          ...
          ?.addEventListener("click", async () => {
            ...
            if (!desc || !amount || !date) {
              errEl.textContent =
                "Descrizione, importo e data sono obbligatori";
            ...
            try {
              ...
              fd.append(
                "category",
                document.getElementById("fex-cat")?.value || "altro",
              );
              ...
              if (fileInp && fileInp.files[0])
                fd.append("receipt", fileInp.files[0]);
      ...
      t.querySelectorAll("[data-del-exp]").forEach((btn) =>
