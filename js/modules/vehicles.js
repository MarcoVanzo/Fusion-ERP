/**
 * Vehicles Module — Gestione Mezzi (Minibuses)
 * Fusion ERP v1.0
 */
...
const Vehicles = (() => {
  ...
  async function init() {
    ...
    try {
      _vehicles = await Store.get('getAllVehicles', 'vehicles');
      renderVehicleList();
    } catch (err) {
      app.innerHTML = Utils.emptyState('Errore nel caricamento dei mezzi', err.message);
  ...
  function renderVehicleList() {
    ...
    const activeVehicles = _vehicles.filter(v => v.status === 'active').length;
    const maintenanceVehicles = _vehicles.filter(v => v.status === 'maintenance').length;
    const totalAnomalies = _vehicles.reduce((sum, v) => sum + (v.open_anomalies || 0), 0);
      ...
      <div class="vehicles-dashboard">
        <div class="dash-top-bar">
          <div>
            ...
            <p class="dash-subtitle">${_vehicles.length} veicoli nel parco mezzi</p>
          </div>
        ...
        <div class="dash-stat-grid">
          <div class="dash-stat-card">
            ...
            <div class="dash-stat-value">${activeVehicles} <span style="font-size:16px; color:rgba(255,255,255,0.5); font-weight:500;">/ ${_vehicles.length}</span></div>
          </div>
          <div class="dash-stat-card orange">
            ...
            <div class="dash-stat-value" style="color:var(--accent-orange);">${maintenanceVehicles}</div>
          </div>
          <div class="dash-stat-card pink">
            ...
            <div class="dash-stat-value" style="color:var(--accent-pink);">${totalAnomalies}</div>
          </div>
        </div>

        ${_vehicles.length === 0 ? Utils.emptyState('Nessun mezzo trovato', 'Aggiungi il primo veicolo al parco mezzi.') : `
          <div class="vehicle-grid">
            ${_vehicles.map(v => {
      let statusLabel = 'Attivo';
      let statusClass = 'status-active';
      if (v.status === 'maintenance') { statusLabel = 'In Manutenzione'; statusClass = 'status-maintenance'; }
      if (v.status === 'out_of_service') { statusLabel = 'Fuori Servizio'; statusClass = 'status-out'; }
              ...
              <div class="vehicle-card" data-id="${v.id}">
                <div class="vehicle-header">
                  <div>
                    ...
                    <div class="vehicle-plate">${Utils.escapeHtml(v.license_plate)}</div>
                  </div>
                ...
                <div class="vehicle-metrics">
                  ...
                  <div class="v-metric">
                    ...
                    <span>Scadenza Bollo: ${v.road_tax_expiry ? Utils.formatDate(v.road_tax_expiry) : 'N/D'}</span>
                  </div>
    ...
    document.getElementById('new-vehicle-btn')?.addEventListener('click', () => showVehicleModal(), { signal: _ac.signal });

    Utils.qsa('.vehicle-card').forEach(card => {
      card.addEventListener('click', () => showVehicleDetail(card.dataset.id), { signal: _ac.signal });
    });
  }
  ...
  function showVehicleModal(vehicle = null) {
    const m = UI.modal({
      title: vehicle ? 'Modifica Mezzo' : 'Nuovo Mezzo',
      body: `
    ...
    document.getElementById('veh-cancel')?.addEventListener('click', () => m.close(), { signal: _ac.signal });

    document.getElementById('veh-save')?.addEventListener('click', async () => {
      ...
      const btn = document.getElementById('veh-save');
      ...
      try {
        ...
        if (vehicle) payload.id = vehicle.id;

        await Store.api(vehicle ? 'updateVehicle' : 'createVehicle', 'vehicles', payload);
        UI.toast(vehicle ? 'Mezzo aggiornato' : 'Mezzo aggiunto', 'success');
        m.close();
        await init();
      } catch (err) {
        document.getElementById('veh-error').textContent = err.message;
        document.getElementById('veh-error').classList.remove('hidden');
  ...
  async function showVehicleDetail(id) {
    ...
    try {
      _currentVehicle = await Store.get('getVehicleById', 'vehicles', { id });
  ...
  function renderVehicleDetailView() {
    ...
    // Active tab state
    let activeTab = 'anomalies'; // default to anomalies if they need attention

    function renderTabContent() {
      if (activeTab === 'info') return renderInfoTab();
      if (activeTab === 'maintenance') return renderMaintenanceTab();
      if (activeTab === 'anomalies') return renderAnomaliesTab();
    }

    const renderWrapper = () => {
      let statusLabel = 'Attivo';
      let statusColor = 'var(--accent-green)';
      if (v.status === 'maintenance') { statusLabel = 'In Manutenzione'; statusColor = 'var(--accent-orange)'; }
      if (v.status === 'out_of_service') { statusLabel = 'Fuori Servizio'; statusColor = '#FF5252'; }
      ...
      app.innerHTML = `
        <style>
          ...
          .v-tabs { display: flex; gap: 8px; padding: 0 24px; border-bottom: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.2); }
          .v-tab { 
            padding: 16px 24px; color: rgba(255,255,255,0.5); font-weight: 700; text-transform: uppercase; letter-spacing: 1px; font-size: 13px;
            cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.2s; display: flex; align-items: center; gap: 8px;
          }
          .v-tab:hover { color: #fff; }
          .v-tab.active { color: var(--accent-cyan); border-bottom-color: var(--accent-cyan); }
        ...
        <div class="detail-header">
          <div class="v-title-wrap">
            ...
            <div class="v-plate-badge">${Utils.escapeHtml(v.license_plate)}</div>
            <h1 class="v-title">${Utils.escapeHtml(v.name)}</h1>
            <div class="v-subtitle"><span style="width:8px;height:8px;border-radius:50%;background:${statusColor};display:inline-block;box-shadow:0 0 10px ${statusColor};"></span> ${statusLabel}</div>
          </div>
          <div style="display:flex; gap:12px;">
        ...
        <div class="v-tabs">
          <div class="v-tab ${activeTab === 'anomalies' ? 'active' : ''}" data-tab="anomalies"><i class="ph ph-warning"></i> Anomalie (${v.anomalies?.filter(a => a.status !== 'resolved').length || 0})</div>
          <div class="v-tab ${activeTab === 'maintenance' ? 'active' : ''}" data-tab="maintenance"><i class="ph ph-wrench"></i> Manutenzione (${v.maintenance?.length || 0})</div>
          <div class="v-tab ${activeTab === 'info' ? 'active' : ''}" data-tab="info"><i class="ph ph-list-dashes"></i> Info e Scadenze</div>
        </div>
      ...
      // Bind events
      document.getElementById('btn-back')?.addEventListener('click', () => init(), { signal: _ac.signal });
      document.getElementById('btn-edit-veh')?.addEventListener('click', () => showVehicleModal(v), { signal: _ac.signal });

      Utils.qsa('.v-tab').forEach(t => t.addEventListener('click', () => {
        activeTab = t.dataset.tab;
        renderWrapper(); // Re-render for simplicity, could optimize
      }, { signal: _ac.signal }));
    ...
    const renderInfoTab = () => `
      <div class="info-grid">
        <div class="info-card">
          ...
          <div class="info-val">${v.capacity} Posti a sedere</div>
          <div class="info-lbl">Stato Attuale</div>
          <div class="info-val">${v.status === 'active' ? 'Attivo' : v.status === 'maintenance' ? 'In Manutenzione' : 'Fuori Servizio'}</div>
          <div class="info-lbl">Note Aggiuntive</div>
          <div class="info-val" style="color:rgba(255,255,255,0.7); font-weight:400; font-size:14px;">${v.notes ? Utils.escapeHtml(v.notes).replace(/\\n/g, '<br>') : '—'}</div>
        </div>
        <div class="info-card">
          ...
          <div class="info-val" style="${isExpiredOrClose(v.insurance_expiry) ? 'color:#FF5252' : ''}">${v.insurance_expiry ? Utils.formatDate(v.insurance_expiry) : 'N/D'}</div>
          <div class="info-lbl">Scadenza Bollo (Road Tax)</div>
          <div class="info-val" style="${isExpiredOrClose(v.road_tax_expiry) ? 'color:#FF5252' : ''}">${v.road_tax_expiry ? Utils.formatDate(v.road_tax_expiry) : 'N/D'}</div>
        </div>
      </div>
    `;
    ...
    const renderMaintenanceTab = () => `
      ...
      ${(!v.maintenance || v.maintenance.length === 0) ? Utils.emptyState('Nessuna manutenzione registrata', 'Non ci sono interventi nello storico per questo mezzo.') : `
        <div class="table-wrapper">
          <table class="table">
            ...
            <tbody>
              ${v.maintenance.map(m => `
                <tr>
                  <td style="font-weight:600;">${Utils.formatDate(m.maintenance_date)}</td>
                  <td>${formatMaintenanceType(m.type)}</td>
                  <td>${m.mileage ? m.mileage.toLocaleString() + ' km' : '—'}</td>
                  <td>${Utils.escapeHtml(m.description || '—')}</td>
                  <td>${m.cost > 0 ? '€ ' + Utils.formatNum(m.cost, 2) : '—'}</td>
                  <td>
                    ${m.next_maintenance_date ? Utils.formatDate(m.next_maintenance_date) : ''}
    ...
    const renderAnomaliesTab = () => `
      ...
      ${(!v.anomalies || v.anomalies.length === 0) ? Utils.emptyState('Nessuna anomalia', 'Tutto funziona regolarmente.') : `
            ...
            <div style="background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); border-left:4px solid ${getSeverityColor(a.severity)}; border-radius:12px; padding:20px; display:flex; justify-content:space-between; align-items:flex-start;">
              <div>
                <div style="display:flex; align-items:center; gap:12px; margin-bottom:8px;">
                  ...
                  <span style="font-size:12px; color:rgba(255,255,255,0.5);"><i class="ph ph-clock"></i> ${Utils.formatDate(a.report_date.split(' ')[0])}</span>
                  <span style="font-size:12px; color:rgba(255,255,255,0.5);"><i class="ph ph-user"></i> ${Utils.escapeHtml(a.reporter_name || 'Utente')}</span>
                </div>
                <div style="font-size:15px; font-weight:600; margin-bottom:8px;">${Utils.escapeHtml(a.description)}</div>
                ${a.resolution_notes ? `<div style="font-size:13px; color:rgba(255,255,255,0.6); margin-top:12px; padding:12px; background:rgba(0,0,0,0.2); border-radius:8px;"><strong>Risoluzione:</strong> ${Utils.escapeHtml(a.resolution_notes)}</div>` : ''}
              </div>
              <div>
                ${a.status !== 'resolved' ? `<select class="form-input anomaly-status-update" data-id="${a.id}" style="width:140px; padding:6px 10px; font-size:13px; height:auto;">
                ...
                </select>` : `<div style="font-size:12px; color:rgba(255,255,255,0.4); text-align:right;">Risolto il<br>${Utils.formatDate(a.resolved_date.split(' ')[0])}</div>`}
              </div>
            </div>
          `).join('')}
        </div>
    ...
    const isExpiredOrClose = (dateStr) => {
      if (!dateStr) return false;
      const d = new Date(dateStr);
      const now = new Date();
      // true se scaduto o scade entro 30gg
    ...
    const getStatusBadge = (status) => {
      if (status === 'open') return '<span style="font-size:11px; font-weight:bold; padding:2px 8px; border-radius:12px; background:rgba(255,0,0,0.1); color:#FF5252; border:1px solid rgba(255,0,0,0.2);">Aperto</span>';
      if (status === 'in_progress') return '<span style="font-size:11px; font-weight:bold; padding:2px 8px; border-radius:12px; background:rgba(255,152,0,0.1); color:#FF9800; border:1px solid rgba(255,152,0,0.2);">In Lavoraz.</span>';
    ...
    const bindTabEvents = () => {
      document.getElementById('btn-add-maint')?.addEventListener('click', () => showAddMaintenanceModal(v.id), { signal: _ac.signal });
      document.getElementById('btn-add-anomaly')?.addEventListener('click', () => showAddAnomalyModal(v.id), { signal: _ac.signal });

      Utils.qsa('.anomaly-status-update').forEach(select => {
        select.addEventListener('change', async (e) => {
          const newStatus = e.target.value;
          const anomId = e.target.dataset.id;

          if (newStatus === 'resolved') {
            // Need notes for resolution
            e.target.value = 'open'; // reset temporarily
            showResolveAnomalyModal(anomId);
          } else {
            // direct update
            try {
              UI.loading(true);
              await Store.api('updateAnomalyStatus', 'vehicles', { id: anomId, status: newStatus });
              UI.toast('Stato aggiornato', 'success');
              await showVehicleDetail(v.id);
            } catch (err) {
              UI.toast(err.message, 'error');
            } finally {
              UI.loading(false);
  ...
  function showAddMaintenanceModal(vehicleId) {
    ...
    document.getElementById('m-cancel')?.addEventListener('click', () => m.close());
    document.getElementById('m-save')?.addEventListener('click', async () => {
      ...
      try {
        ...
        document.getElementById('m-err').textContent = err.message;
        document.getElementById('m-err').classList.remove('hidden');
  ...
  function showAddAnomalyModal(vehicleId) {
    const m = UI.modal({
    ...
    document.getElementById('a-cancel')?.addEventListener('click', () => m.close());
    document.getElementById('a-save')?.addEventListener('click', async () => {
      ...
      document.getElementById('a-save').disabled = true;
      try {
        ...
        document.getElementById('a-err').textContent = err.message;
        document.getElementById('a-err').classList.remove('hidden');
        document.getElementById('a-save').disabled = false;
      }
    });
  }

  function showResolveAnomalyModal(anomalyId) {
    const m = UI.modal({
    ...
    document.getElementById('ar-cancel')?.addEventListener('click', () => m.close());
    document.getElementById('ar-save')?.addEventListener('click', async () => {
      ...
      try {
        ...
        m.close();
        if (_currentVehicle) await showVehicleDetail(_currentVehicle.id);
      } catch (err) {
        document.getElementById('ar-err').textContent = err.message;
        document.getElementById('ar-err').classList.remove('hidden');
        document.getElementById('ar-save').disabled = false;
      }
    });
  }
