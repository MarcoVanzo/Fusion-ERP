...
const EcommerceDB = (() => {
    ...
    function e() {
        if (!t) {
            ...
            t.version(1).stores({ articoli: "++id, nome, categoria, disponibile, importatoIl", statiOrdini: "ordineId, stato, aggiornatoIl", metadati: "chiave" });
        }
        return t;
    }
    ...
    async function ensureMigrated() {
        try {
            ...
            if (count > 0) {
                console.log("EcommerceDB: Trovati " + count + " articoli locali, migrazione in MySQL in corso (a lotti)...");
                ...
                for (let i = 0; i < localArticles.length; i += chunkSize) {
                    ...
                    console.log(`EcommerceDB: Migrati articoli ${i} a ${i + chunk.length}`);
                }
    ...
    return {
        ...
        bulkSaveArticoli: async function(prodotti) {
            const res = await Store.api('bulkSaveProdotti', 'ecommerce', { prodotti });
            if (window.Store && typeof window.Store.invalidate === 'function') {
                Store.invalidate('getProdotti');
            }
            return true;
        ...
        getAllStatiOrdini: async function() {
            const t = await e().statiOrdini.toArray(), o = new Map;
            return t.forEach(t => o.set(String(t.ordineId), t)), o;
        },
        setStatoOrdine: async function(t, o) {
            return e().statiOrdini.put({ ordineId: String(t), stato: o, aggiornatoIl: (new Date).toISOString() });
        },
        ...
        urlToBase64: async function(t) {
            ...
            try {
                const e = new URL(t);
                if (["localhost", "127.0.0.1", "0.0.0.0", "169.254.169.254", "::1", "::"].includes(e.hostname) || e.hostname.endsWith(".local") || e.hostname.endsWith(".internal")) throw new Error("Blocked internal domain.");
                const o = await fetch(t, { mode: "cors", cache: "no-store" });
                if (!o.ok) return null;
                const n = await o.blob();
                return new Promise(t => {
                    const e = new FileReader;
                    e.onloadend = () => t(e.result), e.onerror = () => t(null), e.readAsDataURL(n);
                });
            } catch {
                return null;
            }
        ...
        fileToBase64: async function(t) {
            return new Promise((e, o) => {
                ...
                n.onloadend = () => e(n.result), n.onerror = () => o(new Error("Errore lettura file")), n.readAsDataURL(t);
            });
        }
    };
})();
