...
const Squadre = (() => {
  ...
  function _renderTeams() {
    ...
    if (_teams.length === 0) {
      container.innerHTML = Utils.emptyState(
        "Nessuna squadra",
        "Aggiungi la prima squadra per iniziare a gestire le stagioni.",
      );
      return;
    }
    ...
    let html =
      '<div class="teams-list" style="display:flex;flex-direction:column;gap:var(--sp-3);">';

    _teams.forEach((team) => {
      const activeSeasons = team.seasons
        ? team.seasons.filter((s) => parseInt(s.is_active) === 1)
        : [];
      const inactiveSeasons = team.seasons
        ? team.seasons.filter((s) => parseInt(s.is_active) === 0)
        : [];
                    ...
                    <div style="flex:1;min-width:250px;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:8px">
                            ...
                            `,
                                    ...
                                    .join("")
                                : '<div style="font-size:13px;color:var(--color-text-muted);font-style:italic">Nessuna stagione attiva</div>'
                            }
                        </div>
                    </div>
    ...
    if (isAdmin) {
      ...
      container.querySelectorAll("[data-toggle-season]").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          btn.disabled = true;
          btn.innerHTML =
            '<i class="ph ph-spinner" style="font-size:14px;animation:spin 1s linear infinite;"></i>';

          try {
            await Store.api("toggleSeason", "teams", {
              id: seasonId,
              is_active: isActive,
            });
            UI.toast(
              isActive === "1" ? "Stagione attivata" : "Stagione disattivata",
              "success",
            );
            await _loadData();
          } catch (err) {
  ...
  function _renderSeasons() {
    ...
    const seasonNames = Object.keys(seasonMap).sort((a, b) =>
      b.localeCompare(a),
    ); // newest first
    ...
    seasonNames.forEach((seasonName) => {
      ...
      const unassociatedTeams = _teams.filter(
        (t) => !associatedTeamIds.has(t.id),
      );
    ...
    if (isAdmin) {
      ...
      container.querySelectorAll("[data-del-season]").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          if (
            !confirm(
              `Rimuovere "${teamName}" dalla stagione "${seasonName}"?\nGli atleti e lo staff associati a questa combinazione squadra-stagione verranno scollegati.`,
            )
          )
            return;
          const origHtml = btn.innerHTML;
          ...
          try {
            await Store.api("deleteSeason", "teams", {
              team_season_id: btn.dataset.delSeason,
            });
  ...
  function _openTeamModal(team = null) {
                  ...
                  !isEdit
                    ? (() => {
                        ...
                        const existingSeasons = [
                          ...new Set(
                            _teams.flatMap((t) =>
                              (t.seasons || []).map((s) => s.season),
                            ),
                          ),
                        ].sort((a, b) => b.localeCompare(a));
                        const seasonOptions = existingSeasons
                          .map(
                            (s) =>
    ...
    if (!isEdit) {
      document
        .getElementById("team-initial-season")
      ...
      ?.addEventListener("click", async () => {
        ...
        if (!isEdit) {
          ...
          if (selectVal === "__new__") {
            initialSeasonStr =
              document.getElementById("team-new-season-name")?.value.trim() ||
              "";
          } else {
  ...
  function _openNewSeasonBulkModal() {
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        const seasonStr = document
          .getElementById("bulk-season-name")
          .value.trim();
        const isActive = document.getElementById("bulk-season-active").checked
          ? 1
          : 0;
        const selectedTeamIds = [
          ...document.querySelectorAll(".bulk-team-cb:checked"),
        ].map((cb) => cb.value);
        ...
        for (const teamId of selectedTeamIds) {
          try {
            ...
            const teamName =
              _teams.find((t) => t.id === teamId)?.name || teamId;
        ...
        if (successCount > 0) {
          UI.toast(
            `Stagione "${seasonStr}" creata per ${successCount} squadr${successCount === 1 ? "a" : "e"}`,
            "success",
          );
        }
        if (errors.length > 0) {
  ...
  function _openAddTeamToSeasonModal(seasonName) {
    ...
    if (availableTeams.length === 0) {
      UI.toast(
        "Tutte le squadre sono già associate a questa stagione.",
        "info",
      );
      return;
    }

    const teamCheckboxes = availableTeams
      .map(
        (t) => `
            <label style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--color-surface);border:1px solid var(--color-border);border-radius:6px;cursor:pointer;">
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        const selectedIds = [
          ...document.querySelectorAll(".add-team-cb:checked"),
        ].map((cb) => cb.value);
        const isActive = document.getElementById("add-team-active").checked
          ? 1
          : 0;
        ...
        UI.toast(
          `${ok} squadr${ok === 1 ? "a" : "e"} aggiunte alla stagione "${seasonName}"`,
          "success",
        );
        await _loadData();
        modal.close();
  ...
  return {
    async init() {
      ...
      _currentTab =
        currentRoute === "squadre-stagioni" ? "stagioni" : "squadre";

      container.innerHTML = `
        <div class="transport-dashboard">
            <div class="dash-top-bar">
                <div>
                    <h1 class="dash-title">Gestione <span style="color:var(--accent-pink);">Squadre</span></h1>
                    <p class="dash-subtitle">Configura le squadre e le loro stagioni sportive</p>
                    <!-- Tab bar -->
                    <div class="dash-filters" style="margin-top:24px; margin-bottom:0;">
                        <button class="dash-filter squadre-tab-btn ${_currentTab === "squadre" ? "active" : ""}" data-tab="squadre" type="button">Squadre</button>
                        <button class="dash-filter squadre-tab-btn ${_currentTab === "stagioni" ? "active" : ""}" data-tab="stagioni" type="button">Stagioni</button>
                    </div>
                </div>
                ${
                  isAdmin
                ...
                <div style="display:flex;gap:12px; flex-wrap:wrap;">
                    <button class="btn-dash pink" id="btn-add-team" style="${_currentTab === "squadre" ? "" : "display:none;"}">
                        <i class="ph ph-plus-circle" style="font-size:18px;"></i> NUOVA SQUADRA
                    </button>
                    <button class="btn-dash cyan" id="btn-add-season-bulk" style="${_currentTab === "stagioni" ? "" : "display:none;"}">
                        <i class="ph ph-calendar-plus" style="font-size:18px;"></i> NUOVA STAGIONE
                    </button>
                </div>
                `
                    : ""
        ...
        btn.addEventListener("click", () => {
          ...
          if (addTeamBtn)
            addTeamBtn.style.display =
              btn.dataset.tab === "squadre" ? "" : "none";
          if (addSeasonBtn)
            addSeasonBtn.style.display =
              btn.dataset.tab === "stagioni" ? "" : "none";
          // Update tab visuals
