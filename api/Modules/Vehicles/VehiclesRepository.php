<?php
declare(strict_types=1);

namespace FusionERP\Modules\Vehicles;

use PDO;
use Exception;
use FusionERP\Shared\TenantContext;

require_once __DIR__ . '/../../Shared/Database.php';

class VehiclesRepository
{
    private PDO $db;

    public function __construct()
    {
        $this->db = \FusionERP\Shared\Database::getInstance();
    }

    // ─── VEHICLES ─────────────────────────────────────────────────────────────

    public function getAllVehicles(): array
    {
        $tid = TenantContext::id();
        $stmt = $this->db->prepare("
            SELECT v.*,
                   (SELECT COUNT(*) FROM mv_mezzi_manutenzioni WHERE vehicle_id = v.id) as maintenance_count,
                   (SELECT COUNT(*) FROM mv_mezzi_anomalie WHERE vehicle_id = v.id AND status != 'resolved') as open_anomalies
            FROM mv_mezzi v
            WHERE v.tenant_id = :tid
            ORDER BY v.name ASC
        ");
        $stmt->execute([':tid' => $tid]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getVehicleById(string $id): ?array
    {
        $stmt = $this->db->prepare("SELECT * FROM mv_mezzi WHERE id = :id AND tenant_id = :tid");
        $stmt->execute([':id' => $id, ':tid' => TenantContext::id()]);
        $vehicle = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$vehicle) {
            return null;
        }

        // Fetch maintenance
        $stmt = $this->db->prepare("SELECT * FROM mv_mezzi_manutenzioni WHERE vehicle_id = :id AND tenant_id = :tid ORDER BY maintenance_date DESC");
        $stmt->execute([':id' => $id, ':tid' => TenantContext::id()]);
        $vehicle['maintenance'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Fetch anomalies
        $stmt = $this->db->prepare("
            SELECT a.*, u.full_name as reporter_name
            FROM mv_mezzi_anomalie a
            LEFT JOIN users u ON a.reporter_id = u.id
            WHERE a.vehicle_id = :id AND a.tenant_id = :tid
            ORDER BY a.report_date DESC
        ");
        $stmt->execute([':id' => $id, ':tid' => TenantContext::id()]);
        $vehicle['anomalies'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

        return $vehicle;
    }

    public function createVehicle(array $params): void
    {
        $sql = "INSERT INTO mv_mezzi (id, tenant_id, name, license_plate, capacity, status, insurance_expiry, road_tax_expiry, notes)
                VALUES (:id, :tenant_id, :name, :license_plate, :capacity, :status, :insurance_expiry, :road_tax_expiry, :notes)";
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
    }

    public function updateVehicle(string $id, array $params): bool
    {
        $tid = TenantContext::id();
        // First check if vehicle exists for this tenant
        $check = $this->db->prepare("SELECT COUNT(*) FROM mv_mezzi WHERE id = :id AND tenant_id = :tid");
        $check->execute([':id' => $id, ':tid' => $tid]);
        if ((int)$check->fetchColumn() === 0) {
            return false;
        }

        $params[':id'] = $id;
        $params[':tid'] = $tid;
        $sql = "UPDATE mv_mezzi SET
                    name = :name,
                    license_plate = :license_plate,
                    capacity = :capacity,
                    status = :status,
                    insurance_expiry = :insurance_expiry,
                    road_tax_expiry = :road_tax_expiry,
                    notes = :notes
                WHERE id = :id AND tenant_id = :tid";
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
        return true;
    }

    public function deleteVehicle(string $id): bool
    {
        $stmt = $this->db->prepare("DELETE FROM mv_mezzi WHERE id = :id AND tenant_id = :tid");
        $stmt->execute([':id' => $id, ':tid' => TenantContext::id()]);
        return $stmt->rowCount() > 0;
    }

    // ─── MAINTENANCE ──────────────────────────────────────────────────────────

    public function addMaintenance(array $params): void
    {
        $params[':tenant_id'] = TenantContext::id();
        $sql = "INSERT INTO mv_mezzi_manutenzioni (id, tenant_id, vehicle_id, maintenance_date, type, description, cost, mileage, next_maintenance_date, next_maintenance_mileage)
                VALUES (:id, :tenant_id, :vehicle_id, :maintenance_date, :type, :description, :cost, :mileage, :next_maintenance_date, :next_maintenance_mileage)";
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
    }

    // ─── ANOMALIES ────────────────────────────────────────────────────────────

    public function addAnomaly(array $params): void
    {
        $params[':tenant_id'] = TenantContext::id();
        $sql = "INSERT INTO mv_mezzi_anomalie (id, tenant_id, vehicle_id, reporter_id, description, severity, status)
                VALUES (:id, :tenant_id, :vehicle_id, :reporter_id, :description, :severity, :status)";
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
    }

    public function updateAnomalyStatus(string $id, array $params): bool
    {
        $tid = TenantContext::id();
        $params[':id'] = $id;
        $params[':tid'] = $tid;
        $sql = "UPDATE mv_mezzi_anomalie a
                INNER JOIN mv_mezzi v ON a.vehicle_id = v.id
                SET
                    a.status = :status,
                    a.resolution_notes = :resolution_notes,
                    a.resolved_date = :resolved_date
                WHERE a.id = :id AND a.tenant_id = :tid AND v.tenant_id = :tid";
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
        return $stmt->rowCount() > 0;
    }
}